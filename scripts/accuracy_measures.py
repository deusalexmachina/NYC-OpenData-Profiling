...
df['true_label'] = df['true_label'].map(lambda x: [s.replace(',', '') for s in x.strip().split(', ')])
...
df_results = pd.read_json('task2.json').iloc[1:]
df_results = df_results.explode('semantic_types')
df_results['semantic_type'] = df_results['semantic_types'].apply(lambda x: pd.Series(x['semantic_type'] if x['semantic_type'] != 'other' else x['label']))
