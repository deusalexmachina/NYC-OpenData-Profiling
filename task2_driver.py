...
from similarity import match_preprocess, match_jacc_avg, S_TYPE, MIN_DIST, AVG_DIST, COL
...
def run(gz_paths_cols: List[Tuple[str, str]], ref_set_cols, ref_set_vals):
    ...
    def _match_semantic_vals(col_val, s_type_col, is_spark=True):
        ...
        if not col_val in cache_col_val:
            ...
            if str_col_val == 'null' or str_col_val == '-' or str_col_val == '_' or str_col_val == '0' or str_col_val == 'none' or str_col_val == '' or col_val is None:
                ...
                # print('res0:', res0)
                # res0[MIN_DIST] != 0.0
                if res0 is None or AVG_CUTOFF < res0[AVG_DIST]:  # was the cutoff passed, i.e. was the value present for this semantic type based on the col_name match?
                    ...
                    ref_set_diff[s_type_col] = ref_set_vals[s_type_col]  # prevent key error and delete all values for already matched
                    for rm in remove:
    ...
    match_semantic_vals = udf(_match_semantic_vals, StructType([StructField('s_type_val', StringType(), False), StructField('add', BooleanType(), False)]))
    ...
    def _run(df, i):
        ...
        if i >= -10:
            ### Spark method
            df_cols = df_cols.withColumn('s_type_val_add', match_semantic_vals('value', 's_type_col'))  # match uknown col value to semantic type
            
            # add to ref set with Spark
            df_cols = df_cols.select('*', 's_type_val_add.s_type_val')
            df_cols = df_cols.select('*', 's_type_val_add.add')
            s_types_distinct = df_cols.select('s_type_val').rdd.map(lambda x: x['s_type_val']).countByValue().items()
...
if __name__ == '__main__':
    ...
    if ds_path is None:
        ...
        exit()
    gz_paths_cols: List[Tuple[str, str, str]] = get_task2_dfs(ds_path)
